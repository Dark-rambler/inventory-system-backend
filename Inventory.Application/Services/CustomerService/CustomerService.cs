...
namespace Inventory.Application.Services.CustomerService
{
    public class CustomerService(ICustomerRepository repository, IMapper mapper, IValidator<CustomerRequest> validator) : ICustomerService
    {
        ...
        //public async Task<CustomerResponse> UpdateCustomerAsymc(Guid id, CustomerRequest request) { 
        //await validator.ValidateAndThrowAsync(request);
        //    return mapper.Map<CustomerResponse>(await repository.UpdateCustomerAsync(mapper.Map(request, await FindCustomerById(id))));
        //}
